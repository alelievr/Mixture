...
#ifdef CRT_2D
				float4 noise = float4(GenerateRidgedCellular2DNoise(uvs, _Frequency, _Octaves, _Persistance, _Lacunarity).rrr, 1);
#else
				float4 noise = float4(GenerateRidgedCellular3DNoise(uvs, _Frequency, _Octaves, _Persistance, _Lacunarity).rrr, 1);
#endif
